...
export class OrdersService {
	...
	listApprovalOrders(iduser: string, persona: number, typeOrder: typeOrderEnum): Observable<IApprovalOrder[]> {
		const method = typeOrder === typeOrderEnum.OC
      ? OrdersConstants.LIST_APPROVAL_PURCHASE_ORDER
      : OrdersConstants.LIST_APPROVAL_SERVICES_ORDER;
	...
	getOrderDetail(parameter: string, typeOrder: typeOrderEnum): Observable<IOrderDetail[]> {
		const method = typeOrder === typeOrderEnum.OC
      ? OrdersConstants.GET_PURCHASE_ORDER_DETAILS
      : OrdersConstants.GET_SERVICE_ORDER_DETAILS;
	...
	executeApprovalOrder(request: IExecuteApprovalOrders, typeOrder: typeOrderEnum): Observable<boolean> {
		const method = typeOrder === typeOrderEnum.OC
      ? OrdersConstants.EXECUTE_APPROVAL_PURCHASE_ORDER
      : OrdersConstants.EXECUTE_APPROVAL_SERVICE_ORDER;
